...
DemoDescription['geolocate'] = {title: '地理定位控件', desc: '地理定位控件包装了w3c 的geolocation 接口，Geolocation API标准定义了获取相关设备所提供的地理位置信息的编程接口，这些位置信息的常见来源包括全球定位系统（GPS），以及通过诸如IP地址、RFID、WiFi和蓝牙的MAC地址、和GSM/CDMA手机ID的网络信号所做的推断。该范例与map结合使用；在位置改变时可以响应事件。', oper: '在发布状态下，点击“定位”定位你所在的位置。不支Firefox和Chrome' }
...
DemoDescription['ctl_GraphPie'] = {title: '饼图', desc: '定义：1.本范例展示客户端统计专题图-基础饼图的使用方式，，通过饼图展示 2009 年到 2013 年中国各省级行政区内的居民消费水平（数据来源：国家统计局 http://data.stats.gov.cn/ ）。2. 范例展示统计图 style 快速切换的方法。3.此范例展示通过事件实现与专题要素的交互，显示数据信息。', oper: '点击"添加专题图"制作专题图;  按钮"初始样式"、"高亮填充"、"高亮描边"可以快速修改图表样式；点击"清除"清空专题图层；鼠标移动到图表上可以查看数据信息。' }
